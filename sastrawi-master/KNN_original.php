  ...
  <nav class="navbar navbar-inverse navbar-fixed-top">
      <div class="container">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="#">TF IDF KNN</a>
        </div>
    ...
    <div class="col-md-12">
	<center>
	<h3>Kategorisasi Teks Bahasa Indonesia Menggunakan TF IDF - KNN</h3>
	<form action="KNN_original.php" method="post" enctype="multipart/form-data">
	...
	for($i=0;$i<count($words);$i++){
			...
			$tfidf_1[$words[$i]]=$jumlah_tf1[$words[$i]]*log10(6/$jumlah_df_semua[$words[$i]]);
			$tfidfsq_1[$words[$i]]=($tfidf_1[$words[$i]])*($tfidf_1[$words[$i]]);
			$tfidf_2[$words[$i]]=$jumlah_tf2[$words[$i]]*log10(6/$jumlah_df_semua[$words[$i]]);
			$tfidfsq_2[$words[$i]]=($tfidf_2[$words[$i]])*($tfidf_2[$words[$i]]);
			$tfidf_3[$words[$i]]=$jumlah_tf3[$words[$i]]*log10(6/$jumlah_df_semua[$words[$i]]);
			$tfidfsq_3[$words[$i]]=($tfidf_3[$words[$i]])*($tfidf_3[$words[$i]]);
			$tfidf_4[$words[$i]]=$jumlah_tf4[$words[$i]]*log10(6/$jumlah_df_semua[$words[$i]]);
			$tfidfsq_4[$words[$i]]=($tfidf_4[$words[$i]])*($tfidf_4[$words[$i]]);
			$tfidf_5[$words[$i]]=$jumlah_tf5[$words[$i]]*log10(6/$jumlah_df_semua[$words[$i]]);
			$tfidfsq_5[$words[$i]]=($tfidf_5[$words[$i]])*($tfidf_5[$words[$i]]);
			$tfidf_6[$words[$i]]=$jumlah_tf6[$words[$i]]*log10(6/$jumlah_df_semua[$words[$i]]);
			$tfidfsq_6[$words[$i]]=($tfidf_6[$words[$i]])*($tfidf_6[$words[$i]]);
			$tfidf_query[$words[$i]]=$jumlah_tf_query[$words[$i]]*log10(6/$jumlah_df_semua[$words[$i]]);
			$tfidfsq_query[$words[$i]]=($tfidf_query[$words[$i]])*($tfidf_query[$words[$i]]);
			}
		else {
			$tfidf_1[$words[$i]]=0;
			$tfidfsq_1[$words[$i]]=0;
			...
			$tfidf_6[$words[$i]]=0;
			$tfidfsq_6[$words[$i]]=0;
			$tfidf_query[$words[$i]]=0;
			$tfidfsq_query[$words[$i]]=0;
			}	
		} 
	...
	$product=array($cossim_of_dok1,$cossim_of_dok2,$cossim_of_dok3,$cossim_of_dok4,$cossim_of_dok5,$cossim_of_dok6);
	$dok=array('Finance',' Finance','Sport','Politic', 'Daily  News',' Daily News');
	$class_product=array_combine($dok,$product);
	arsort($class_product);
	foreach ($class_product as $word=>$count){
		$word_rank[]=$word;		
		}	
	...
	echo "<div class='col-md-12'><center><h4>So based on TF IDF KNN computation, Document Query is classified into $word_rank[0]</h4></center></div>";
	}
	?>
<div class="col-md-12">
	<p>
